...
Page({
  ...
  upload(params, type) {
    console.log(params)
    wx.request({
      url: `https://api.ai.qq.com/fcgi-bin/vision/vision_objectr`, // 仅为示例，并非真实的接口地址
      data: params,
      method: 'POST',
      header: {
      ...
      success: (res) => {
        ...
        if (res.data.ret != 0) {
          ...
          let content = ''
          res.data.data.object_list.forEach((item) => {
            content += item.label_id,
            content += item.label_confd
          })
